...
def open_leaderboard_window():
    ...
    leaderboard_window = tk.Toplevel(app)
    leaderboard_window.title("Таблица лидеров")
    leaderboard_window.geometry("400x400")

    # Заголовок формы
    leaderboard_label = tk.Label(leaderboard_window, text="Таблица лидеров", font=("Arial", 20))
...
def open_help_window():
    ...
    # Заголовок формы
    help_label = tk.Label(help_window, text="Инструкции для новичков", font=("Arial", 20))
    ...
    """
    instructions_label = tk.Label(help_window, text=instructions, font=("Arial", 12), justify="left")
    instructions_label.pack(pady=10)

    # Переключатель для подсказок
    hint_label = tk.Label(help_window, text="Включить подсказки:")
    hint_label.pack(pady=(10, 5))

    # Создание списка для словаря
    word_list_label = tk.Label(help_window, text="Статичный словарь:", font=("Arial", 12))
    word_list_label.pack(pady=10)

    # Создание Scrollable Frame
    scrollable_frame = tk.Frame(help_window)
    scrollable_frame.pack(pady=10, fill="both", expand=True)

    # Добавляем Listbox для отображения слов
    word_listbox = tk.Listbox(scrollable_frame, height=10, width=35)
    word_listbox.pack(padx=10, pady=5)

    # Пример словаря
    word_dict = {
        "ИГРА": "Процесс выполнения действий в игре",
        "СЛОВО": "Единица языка",
    }
    ...
    user_word_label = tk.Label(help_window, text="Введите слово для проверки:", font=("Arial", 12))
    user_word_label.pack(pady=5)

    user_word_entry = tk.Entry(help_window, width=20)
    ...
    check_button = tk.Button(help_window, text="Проверить слово", command=check_word)
    check_button.pack(pady=10)

    # Метка для отображения результата проверки слова
    result_label = tk.Label(help_window, text="", font=("Arial", 12))
...
def open_user_window():
    ...
    # Заголовок формы
    welcome_label = tk.Label(user_window, text="Добро пожаловать", font=("Arial", 20))
    welcome_label.pack(pady=10)

    # Метка и поле ввода email
    email_label = tk.Label(user_window, text="E-mail:")
    email_label.pack(pady=(10, 5))
    email_entry = tk.Entry(user_window, width=30)
    ...
    # Кнопка "Войти"
    submit_button = tk.Button(user_window, text="Войти", command=lambda: print("Email:", email_entry.get(), "Пароль:", password_entry.get()))
    submit_button.pack(pady=(5, 10))

    # Кнопка "Забыл пароль"
    forgot_password_button = tk.Button(user_window, text="Забыл пароль", command=lambda: print("Забыл пароль"))
    forgot_password_button.pack(pady=(5, 10))

    # Кнопка "Регистрация"
    register_button = tk.Button(user_window, text="Регистрация", command=lambda: print("Регистрация"))
...
top_frame = tk.Frame(master=app)
top_frame.pack(side="top", fill="x", padx=0, pady=0)

# Размещение 4 кнопок в одном ряду внутри top_frame с помощью grid
button1 = tk.Button(master=top_frame, text="Пользователь", command=open_user_window)
button1.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")

button2 = tk.Button(master=top_frame, text="Режим игры", command=lambda: button_click(2))
button2.grid(row=0, column=1, padx=10, pady=10, sticky="nsew")

button3 = tk.Button(master=top_frame, text="Помощь", command=open_help_window)
button3.grid(row=0, column=2, padx=10, pady=10, sticky="nsew")

button4 = tk.Button(master=top_frame, text="Таблица лидеров", command=open_leaderboard_window)
...
letter_tile_frame = tk.Frame(master=app)
...
for index, letter in enumerate(letters):
    letter_button = tk.Button(master=letter_tile_frame, text=letter, command=lambda letter=letter: add_letter(letter))
    letter_button.grid(row=index // 6, column=index % 6, padx=5, pady=5, sticky="nsew")

# Создание фрейма для кнопки "Начать игру"
bottom_frame = tk.Frame(master=app)
bottom_frame.pack(side="bottom", fill="x", padx=10, pady=10)

# Кнопка "Начать игру"
start_button = tk.Button(master=bottom_frame, text="Начать игру", command=start_game)
start_button.pack()

# Кнопка "Удалить букву"
backspace_button = tk.Button(master=bottom_frame, text="Удалить букву", command=remove_letter)
...
word_label = tk.Label(master=app, textvariable=current_word, font=("Arial", 16))
