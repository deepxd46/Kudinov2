...
'''
PCA фокусируется на максимальном сохранении общей дисперсии данных путём поиска ортогональных линейных комбинаций, 
при этом не разделяя шумовую составляющую.
Факторный анализ стремится выявить скрытые латентные переменные, объясняющие корреляционную 
структуру между наблюдаемыми переменными, 
отделяя общую дисперсию от уникальной (шумовой).
...
называемых факторами: в один фактор объединяются переменные, сильно коррелирующие между собой.
'''
